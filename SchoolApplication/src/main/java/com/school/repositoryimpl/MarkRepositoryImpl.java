...
public class MarkRepositoryImpl implements MarkRepository{
	...
	public List<MarkEntity> getAllTermMarks(Long rollNo) throws DatabaseException
	{
        ...
        public List<MarkEntity> getAllStudentMarks(Long roomNo) throws DatabaseException
		{
			logger.debug("In Retrieving All Mark Details...");
			Session session = null;
			List<MarkEntity> markList =new ArrayList<MarkEntity>();
				...
				for(int i=0;i<markList.size();i++)
				{
					if(markList.get(i).getTermType().equals("I") || markList.get(i).getTermType().equals("II") || markList.get(i).getTermType().equals("III")) {
						if(markList.get(i).getTamil()>=0 && markList.get(i).getEnglish()>=0 && markList.get(i).getMaths()>=0 && markList.get(i).getScience()>=0 && markList.get(i).getSocialScience()>=0)
						{
							...
							resultRepository.updateTermMarks(markList.get(i).getTermType(),totalMarks,termStatus, markList.get(i).getStudentEntity().getRollNo());
						}
					}
				}
